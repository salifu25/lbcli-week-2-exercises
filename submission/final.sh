...
DECODED_BASE_TX=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX")
TXID=$(echo "$DECODED_BASE_TX" | jq -r '.txid')
...
# WRITE YOUR SOLUTION BELOW:
NUM_INPUTS=$(echo "$DECODED_BASE_TX" | jq -r '.vin | length')
check_cmd "Input counting" "NUM_INPUTS" "$NUM_INPUTS"

NUM_OUTPUTS=$(echo "$DECODED_BASE_TX" | jq -r '.vout | length')
...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(echo "$DECODED_BASE_TX" | jq -r '.vout[0].value * 100000000 | round')
...
UTXO_VALUE=$(echo "$DECODED_BASE_TX" | jq -r ".vout[$UTXO_VOUT_INDEX].value * 100000000 | round")
...
PAYMENT_BTC=$(jq -rn --argjson s "$PAYMENT_AMOUNT" '$s / 100000000')
CHANGE_BTC=$(jq -rn --argjson s "$CHANGE_AMOUNT" '$s / 100000000')
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(echo "$DECODED_TX" | jq -r --arg a "$CHANGE_ADDRESS" '.vout[] | select(.scriptPubKey.address == $a) | .n')
...
# Convert to BTC
CHILD_SEND_BTC=$(jq -rn --argjson s "$CHILD_SEND_AMOUNT" '$s / 100000000')
...
DECODED_SECONDARY_TX=$(bitcoin-cli -regtest decoderawtransaction "$SECONDARY_TX")
SECONDARY_TXID=$(echo "$DECODED_SECONDARY_TX" | jq -r '.txid')
...
# WRITE YOUR SOLUTION BELOW:
TIMELOCK_VOUT=1
TIMELOCK_INPUTS='[{"txid":"'"$SECONDARY_TXID"'","vout":'"$TIMELOCK_VOUT"',"sequence":10}]'
...
SECONDARY_OUTPUT_VALUE=$(echo "$DECODED_SECONDARY_TX" | jq -r ".vout[$TIMELOCK_VOUT].value * 100000000 | round")
...
# Convert to BTC
TIMELOCK_BTC=$(jq -rn --argjson s "$TIMELOCK_AMOUNT" '$s / 100000000')
